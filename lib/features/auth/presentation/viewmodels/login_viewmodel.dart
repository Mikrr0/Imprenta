...
class LoginViewModel extends ChangeNotifier {
  ...
  Future<void> restaurarSesionDesdeFirebase() async {
    ...
    try {
      final perfil = await loginUseCase.authRepository
          .obtenerUsuarioStream(user.uid)
          .first;

      usuarioActual = perfil;
      _iniciarEscuchaSesionViva(user.uid);
    } catch (e) {
      usuarioActual = null;
        ...
        .listen(
          (perfilSincronizado) async {
            ...
            try {
              ...
              if (errorStr.contains('user-disabled') ||
                  errorStr.contains('auth/user-disabled')) {
                _ejecutarCierreDeSesionForzado(
                  "Esta cuenta ha sido inhabilitada por el administrador.",
                );
                return;
              }
            }
            ...
            if (usuarioActual != null) {
              if (perfilSincronizado.rol != usuarioActual!.rol ||
                ...
                _ejecutarCierreDeSesionForzado(
                  "Tu sesión fue cerrada por cambios de permisos en el servidor.",
                );
                return;
              }
            }
          ...
          onError: (error) {
            if (error.toString().contains('CUENTA_INHABILITADA')) {
              _ejecutarCierreDeSesionForzado(
                "Tu cuenta ha sido inhabilitada por el administrador.",
              );
            } else {
              _ejecutarCierreDeSesionForzado(
                "Inconsistencia detectada en los datos de sesión.",
              );
            }
          },
        );
  ...
  Future<bool> procesarInicioDeSesion(
    String rutIngresado,
    ...
    if (bloqueado) {
      estaCargandoDatos = false;
      mensajeDeErrorVisible =
          "Por seguridad, cuenta bloqueada por 15 minutos tras 5 intentos fallidos.";
      notifyListeners();
      return false;
    }
    ...
    try {
      ...
      if (e.toString().contains('CUENTA_INHABILITADA')) {
        mensajeDeErrorVisible =
            "Esta cuenta ha sido inhabilitada. Contacta a administración.";
        notifyListeners();
        return false;
      }

      int intentosActuales = await _securityService.registrarIntentoFallido(
        rutLimpio,
      );

      if (intentosActuales >= 5) {
        mensajeDeErrorVisible =
            "Cuenta bloqueada temporalmente por 15 minutos tras 5 intentos fallidos.";
      } else {
        mensajeDeErrorVisible =
            "Credenciales incorrectas o usuario no registrado. Intento $intentosActuales de 5.";
      }
    ...
    try {
      ...
      if (errorCrudo.contains('email-already-in-use') ||
        ...
        mensajeDeErrorVisible = "La contraseña ingresada es demasiado débil.";
          ...
          errorCrudo.contains('connection')) {
        mensajeDeErrorVisible =
            "Error de conexión. Revisa tu acceso a internet.";
      } else {
        mensajeDeErrorVisible =
            "Ocurrió un error en el servidor al intentar registrar.";
      }
